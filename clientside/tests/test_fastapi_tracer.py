...
async def test_trace_endpoint_with_exception():
    with patch("src.captureflow.tracer.Tracer._send_trace_log") as mock_log:
        ...
        exception_events = [
            e for e in log_data["execution_trace"] if e["event"] == "exception"
        ]
        assert (
            len(exception_events) > 0
        ), "Exception event not found in the execution trace"
        exception_event = exception_events[0]
        assert (
            "ZeroDivisionError" in exception_event["exception_info"]["type"]
        ), "Expected ZeroDivisionError"
        assert (
            "division by zero" in exception_event["exception_info"]["value"]
